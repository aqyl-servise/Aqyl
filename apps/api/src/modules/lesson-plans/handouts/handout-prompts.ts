...
export function buildHandoutPrompt(opts: HandoutPromptOpts): { system: string; user: string } {
  ...
  const teacherRule = isAssessed
    ? `Это ОЦЕНИВАЕМОЕ задание на ${points ?? '—'} балл(ов). В teacherExtra дай ключи/ответы (answers) ` +
      ...
      // Частые арифметические ошибки по логам валидатора (ТЗ 1.5.2): каждая
      // стоит перегенерации блока. Дешевле назвать их до ответа.
      `ПРОВЕРЬ АРИФМЕТИКУ ДО ОТВЕТА:\n` +
      `1) баллы за пункты по отдельности → строк дескриптора ровно столько, сколько ` +
      `оцениваемых пунктов, каждая про СВОЙ пункт с ЕГО баллом. Одна строка «по 1 баллу ` +
      `за каждый» на несколько пунктов — ошибка;\n` +
      `2) сумма баллов всех строк дескриптора равна totalPoints;\n` +
      `3) пересчитай пропуски «____» в готовом задании: ровно это число ставь в items.gaps ` +
      `и называй в критериях;\n` +
      `4) пороговая шкала: диапазоны от 0 до максимума, без дыр и пересечений, больше ` +
      `верных — не меньше баллов, верхний диапазон покрывает максимум.`
...
export function buildScoringFixPrompt(opts: {
  ...
  return {
    ...
    user:
      ...
      `Факты выше посчитаны кодом по готовому тексту — они ИСТИНА, не пересчитывай их ` +
      `по-своему: в items проставь gaps так, чтобы их сумма была ровно ${gapCount}, ` +
      `и в тексте критериев называй именно эти числа.\n` +
