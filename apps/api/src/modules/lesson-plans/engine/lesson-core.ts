...
export function checkFactYears(text: string, facts: CoreFact[]): FactProblem[] {
  ...
  // Узкое окно: маркер и год должны стоять связкой («туған жылы 1894»,
  // «1894 жылы туған»). На широком окне многозначные слова притягивали чужие
  // годы: «Туған өңірінің табиғаты…  1937 жылы тұтқындалды» — «туған» здесь
  // про родной край, а не про год рождения (реальный случай приёмки).
  const RADIUS = 30;
