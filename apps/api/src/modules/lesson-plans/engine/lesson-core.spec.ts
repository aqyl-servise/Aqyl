...
test('C1: верные даты, дистракторы и чужие годы не дают ложных срабатываний', () => {
  ...
  // Многозначный маркер: «Туған өңір» — родной край, а не год рождения.
  assert.equal(
    checkFactYears('Туған өңірінің табиғаты өлеңінің рухына әсер етті 1937 жылы тұтқындалды', SEIFULLIN_FACTS).length,
    0, '«туған» в значении «родной» не притягивает чужой год',
  );
  // Обратный порядок связки по-прежнему ловится.
  assert.equal(checkFactYears('Ақын 1901 жылы туған.', SEIFULLIN_FACTS).length, 1);
});

test('C1: год из листа не противоречит листу, даже рядом с чужим маркером', () => {
  // Регрессия золотого набора: в биографии годы идут подряд, и ближайший
  // маркер приписывал верный год чужому атрибуту («год смерти — 1937»,
  // хотя 1937 это год ареста, тоже из листа).
  const facts: CoreFact[] = [
    ...SEIFULLIN_FACTS,
    { entity: 'С. Сейфуллин', attribute: 'тұтқынға алынуы', value: '1937', claim: '1937 жылы тұтқындалған', confidence: 'high' },
  ];
  assert.equal(
    checkFactYears('Ақын 1894 жылы туған, 1937 жылы тұтқындалып, 1938 жылы қайтыс болған.', facts).length,
    0, 'все три года есть в листе — противоречия нет',
  );
  // Выдуманный год по-прежнему ловится.
  assert.equal(checkFactYears('Сейфуллин туған жылы 1901.', facts).length, 1);
});

test('C1: плотный список годов — варианты ответа, не утверждения', () => {
  assert.equal(
    checkFactYears('Ом заңы қай жылы ашылды? 1789, 1826, 1854',
      [{ entity: 'Ом заңы', attribute: 'ашылған жылы', value: '1826', claim: '1826 жылы ашылды', confidence: 'high' }],
    ).length,
    0, 'три года подряд — список вариантов',
  );
});
