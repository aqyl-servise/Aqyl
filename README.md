...
## После деплоя — первый вход

При первом запуске API автоматически создаётся единственный аккаунт администратора:
...
1. Войдите как `admin@aqyl.kz` / `admin123`
2. Перейдите в **Профиль** и смените пароль на надёжный
3. В разделе **Заявки** вы можете одобрять новые регистрации
4. В разделе **Пользователи** создайте аккаунты для педагогического состава или предложите им зарегистрироваться самостоятельно (кнопка «Зарегистрироваться» на странице входа) — заявки будут ожидать вашего одобрения

> Никаких тестовых данных (учителей, классов, учеников) не создаётся автоматически.
> Все данные вводятся через интерфейс администратора.
...
После первого запуска создаётся аккаунт администратора: `admin@aqyl.kz` / `admin123`
